...
/**
 * Instantiates a new Starling_MLX90393 class instance
 */
Starling_MLX90393::Starling_MLX90393(void) {}
 ...
 */
bool Starling_MLX90393::begin_I2C(uint8_t i2c_addr, TwoWire *wire) {
  if (i2c_dev) {
    delete i2c_dev;
  }
 ...
 *    @return True if initialization was successful, otherwise false.
 */
boolean Starling_MLX90393::begin_SPI(uint8_t cs_pin, SPIClass *theSPI) {
  i2c_dev = NULL;
  if (!spi_dev) {
    _cspin = cs_pin;
...
bool Starling_MLX90393::_init(void) {
 ...
 */
bool Starling_MLX90393::exitMode(void) {
 ...
 * @return True if the operation succeeded, otherwise false.
 */
bool Starling_MLX90393::reset(void) {
 ...
 * @return True if the operation succeeded, otherwise false.
 */
bool Starling_MLX90393::setGain(mlx90393_gain_t gain) {
  _gain = gain;
 ...
 */
mlx90393_gain_t Starling_MLX90393::getGain(void) {
  uint16_t data;
  readRegister(MLX90393_CONF1, &data);
 ...
 * @return True if the operation succeeded, otherwise false.
 */
bool Starling_MLX90393::setResolution(enum mlx90393_axis axis,
...
Starling_MLX90393::getResolution(enum mlx90393_axis axis) {
  switch (axis) {
  case MLX90393_X:
    return _res_x;
  case MLX90393_Y:
 ...
 * @return True if the operation succeeded, otherwise false.
 */
bool Starling_MLX90393::setFilter(enum mlx90393_filter filter) {
  _dig_filt = filter;
 ...
 */
enum mlx90393_filter Starling_MLX90393::getFilter(void) { return _dig_filt; }
 ...
 * @return True if the operation succeeded, otherwise false.
 */
bool Starling_MLX90393::setOversampling(
 ...
 */
enum mlx90393_oversampling Starling_MLX90393::getOversampling(void) {
  return _osr;
}
 ...
 * @return True if the operation succeeded, otherwise false.
 */
bool Starling_MLX90393::setTrigInt(bool state) {
  uint16_t data;
...
/**
 * Enable temperature drift compensation.
 * @return True if the operation succeeded, otherwise false.
 */
bool Starling_MLX90393::enableTemperatureCompensation(bool state) {
  uint16_t data;
  readRegister(MLX90393_CONF3, &data);

  // Set TCMP_EN bit to 1
  data &= ~0x5;
  if(state){
    data |= 1 << 10;
  }else{
...
/**
 * Enable Z-series Hall plate measurement
 * @return True if the operation succeeded, otherwise false.
 */
bool Starling_MLX90393::enableZSeriesConnection(bool state) {
  uint16_t data;
  readRegister(MLX90393_CONF1, &data);

  // Set Z_SERIES bit to 1
  data &= ~0x0100;
  if(state){
    data |= 1 << 7;
  } else{
 ...
 */
bool Starling_MLX90393::startSingleMeasurement(void) {
 ...
 * @return True on command success
 */
bool Starling_MLX90393::readMeasurement(float *x, float *y, float *z) {
 ...
 * @return True if the operation succeeded, otherwise false.
 */
bool Starling_MLX90393::readData(float *x, float *y, float *z) {
...
bool Starling_MLX90393::writeRegister(uint8_t reg, uint16_t data) {
  uint8_t tx[4] = {
      MLX90393_REG_WR,
...
bool Starling_MLX90393::readRegister(uint8_t reg, uint16_t *data) {
  uint8_t tx[2] = {
      MLX90393_REG_RR,
      (uint8_t)(reg << 2)}; // the register itself, shift up by 2 bits!
...
*/
/**************************************************************************/
bool Starling_MLX90393::getEvent(sensors_event_t *event) {
  /* Clear the event */
 ...
 */
uint8_t Starling_MLX90393::transceive(uint8_t *txbuf, uint8_t txlen,
...
*/
/**************************************************************************/
void Starling_MLX90393::getSensor(sensor_t *sensor) {
